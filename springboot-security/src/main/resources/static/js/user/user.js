...
async function init() {	
	...
	if (response == null) {
		...
		if (response.code == 200) {
			...
			let userInfoHTML = document.getElementById("userinfo");
			
			userInfoHTML.innerHTML = `
				<div class="col-6">
					<strong>이름:</strong> <span>${response.data.object.name}</span>
				</div>`;
			userInfoHTML.innerHTML += `
				<div class="col-6">
					<strong>이메일:</strong> <span>${response.data.object.email}</span>
				</div>`;
			userInfoHTML.innerHTML += `
				<div class="col-6">
					<strong>가입일:</strong> <span>${(new Date(response.data.object.frstRegDate)).toLocaleString("ko-KR")}</span>
				</div>`;
			userInfoHTML.innerHTML += `
				<div class="col-6">
					<strong>총 주문 횟수:</strong> <span>${response.data.object.totalOrderCount}</span>
				</div>`;
			userInfoHTML.innerHTML += `
				<div class="col-6">
					<strong>총 주문 금액:</strong> <span>${response.data.object.totalOrderMoney}</span>
				</div>`;
			userInfoHTML.innerHTML += `
				<div class="col-6">
					<strong>총 반품 횟수:</strong> <span>${response.data.object.totalReturnCount}</span>
					</div>`;
		} else {
